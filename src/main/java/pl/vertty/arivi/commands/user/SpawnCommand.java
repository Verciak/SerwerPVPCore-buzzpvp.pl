...
public class SpawnCommand extends PlayerCommand
{
    ...
    public boolean onCommand(final Player p, final String[] args) {
        if (args.length != 1) {
            TimerUtil.teleportSpawn(p, Server.getInstance().getDefaultLevel().getSpawnLocation().getLocation(), 7);
            return true;
        }
        ...
        for(Effect e : o.getEffects().values()){
            o.removeEffect(e.getId());
        }
        o.setHealth(p.getMaxHealth());
        TimerUtil.teleportSpawn(o, Server.getInstance().getDefaultLevel().getSpawnLocation().getLocation(), 7);
